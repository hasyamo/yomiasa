...
- ※実装注: counts は記事の `publishedAt` で覚醒前/後を判定できる。閾値定数は `KITACOLE_RANKS = [{rank:'S級覚醒',min:0,boss:null},{rank:'国家級',min:600,boss:'REQUIEM OF SHADOWS'},{rank:'君主前',min:1200,boss:'CAEL NOX'},{rank:'君主',min:2000,boss:'ASHEN REAPER'}]` の形で持つ。
...
- ※作問は手作業。黎明期58記事のタイトル/本文は `.claude/kitacole_wai_counts.csv`（タイトル・key・URL）と本文取得で参照可能。問題データは別ファイル（例 `kitacole_quiz.json`）に持ち、アプリ同梱。
